package s3.td7.arithm;

abstract class Expression {
    
    abstract boolean estOperande();
    abstract boolean estOperation();
    abstract boolean estOperationUnaire();
    abstract boolean estOperationBinaire();
    abstract int getValeur();
    abstract char getOperateur();
    
}

class OperationBinaire extends Expression {
    private char op;
    private Expression expG;
    private Expression expD;

    boolean estOperande() {
        return
    }
    boolean estOperation() {
    
    }
    boolean estOperationUnaire() {
        
    }
    boolean estOperationBinaire() {
       
    }
    
    int getValeur() {
        
    }
    
    char getOperateur() {
        
    }
}

class OperationUnaire extends Expression {
    private char op;
    private Expression exp;
    
    boolean estOperande() {
        return false;
    }
    boolean estOperation() {
        return true;
    }
    boolean estOperationUnaire() {
        
    }
    boolean estOperationBinaire() {
       
    }

    int getValeur() {
        
    }

    char getOperateur() {
        
    }
}

class Operande extends Expression{
    private int val;

    boolean estOperande() {
        return
    }
    boolean estOperation() {
    
    }
    boolean estOperationUnaire() {
        
    }
    boolean estOperationBinaire() {
       
    }

    int getValeur() {
        
    }

    char getOperateur() {
        
    }
}